...
- We dynamically allocate the memory of max buffer size, _CMDLINE MAX_ to _char* cmd_.
- Return the C string with _'\0'_ to notify the library to where the string ends.
...
- We finally return the collected exit value.

### Pipeline Commands
Once the command string is parsed and the string is detected as cotaining the _pipe dilimiter_, "|", we count numbers of pipe in order to check how many arguments is input-ed. Then we pass the parsed string, numbers of pipe delimiter to _execute pipe_.
- 
